...
app.post('/conversations', async (req: Request, res: Response): Promise<void> => {
  try {
...
app.get('/files/:wallet', async (req: Request, res: Response) => {
  try {
    const { wallet } = req.params;
    if (!wallet) {
      res.status(400).json({ error: 'Wallet requerida.' });
...
app.get('/files/:wallet/:fileId', async (req: Request, res: Response) => {
  try {
...
app.delete('/files/:wallet/:fileId', async (req: Request, res: Response) => {
  try {
    const { wallet, fileId } = req.params;
    if (!wallet || !fileId) {
      res.status(400).json({ error: 'Wallet y fileId requeridos.' });
